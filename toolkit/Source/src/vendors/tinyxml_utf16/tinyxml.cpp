...
/*wchar_t* GetCharBufferFromFile(FILE* file, long& length, TiXmlDocument* thisDoc)
{
	...
	if ( length <= 0 )
	{
		thisDoc->SetError( TiXmlBase::TIXML_ERROR_DOCUMENT_EMPTY, 0, 0, TIXML_ENCODING_UNKNOWN );
		return 0;
	}
	...
	if ( fread(buf, length * sizeof(wchar_t), 1, file ) != 1 ) {
		delete [] buf;
		thisDoc->SetError( TiXmlBase::TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
		return 0;
	}
	buf[length] = 0;
	return buf;
...
bool TiXmlDocument::LoadFile( const wchar_t * content, TiXmlEncoding encoding )
{
	...
	const long length = wcslen(content);

	TIXML_STRING data;
	data.reserve( length );

	const wchar_t* lastPos = content;
