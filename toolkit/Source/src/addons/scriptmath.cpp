...
double myMax(const double a, double float b)
{
	return (a>b) ? a : b;
}
#endif
...
void RegisterScriptMath_Native(asIScriptEngine *engine)
{
	...
	// Added by santee in 2010
	r = engine->RegisterGlobalFunction("float max(const float, const float)", asFUNCTIONPR(myMax, (const float, const float), float), asCALL_CDECL); assert( r >= 0 );
	r = engine->RegisterGlobalFunction("float min(const float, const float)", asFUNCTIONPR(myMin, (const float, const float), float), asCALL_CDECL); assert( r >= 0 );
	r = engine->RegisterGlobalFunction("int max(const int, const int)", asFUNCTIONPR(myMax, (const int, const int), int), asCALL_CDECL); assert( r >= 0 );
	r = engine->RegisterGlobalFunction("int min(const int, const int)", asFUNCTIONPR(myMin, (const int, const int), int), asCALL_CDECL); assert( r >= 0 );
...
asDECLARE_FUNCTION_WRAPPERPR(maxf_generic, myMax, (const float, const float), float);
asDECLARE_FUNCTION_WRAPPERPR(minf_generic, myMin, (const float, const float), float);
asDECLARE_FUNCTION_WRAPPERPR(max_generic, myMax, (const int, const int), int);
asDECLARE_FUNCTION_WRAPPERPR(min_generic, myMin, (const int, const int), int);
...
void RegisterScriptMath_Generic(asIScriptEngine *engine)
{
	...
	// max/min
	r = engine->RegisterGlobalFunction("float min(const float, const float)", asFUNCTION(minf_generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterGlobalFunction("float max(const float, const float)", asFUNCTION(maxf_generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterGlobalFunction("int min(const int, const int)", asFUNCTION(min_generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterGlobalFunction("int max(const int, const int)", asFUNCTION(max_generic), asCALL_GENERIC); assert( r >= 0 );
