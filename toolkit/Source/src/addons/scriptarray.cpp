...
void CScriptArray::Resize(asUINT numElements)
{
	...
	Resize(numElements - buffer->numElements, (asUINT)-1);
}
...
asDECLARE_FUNCTION_WRAPPER(__InsertAt, CScriptArray::InsertAt);
asDECLARE_FUNCTION_WRAPPER(__RemoveAt, CScriptArray::RemoveAt);
asDECLARE_FUNCTION_WRAPPER(__InsertLast, CScriptArray::InsertLast);
asDECLARE_FUNCTION_WRAPPER(__RemoveLast, CScriptArray::RemoveLast);
...
static void RegisterScriptArray_Generic(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("array<T>", "void insertAt(uint, const T&in)",	asFUNCTION(__InsertAt), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void removeAt(uint)",				asFUNCTION(__RemoveAt), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void insertLast(const T&in)",		asFUNCTION(__InsertLast), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void removeLast()",				asFUNCTION(__RemoveLast), asCALL_GENERIC); assert( r >= 0 );
}
