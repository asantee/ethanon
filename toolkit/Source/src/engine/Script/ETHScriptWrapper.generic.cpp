...
asDECLARE_FUNCTION_WRAPPER(__FadeOutSample,   ETHScriptWrapper::FadeOutSample);
...
void ETHScriptWrapper::RegisterGlobalFunctions(asIScriptEngine *pASEngine)
{
	...
	r = pASEngine->RegisterGlobalFunction("bool FadeOutSample(const string &in, const float milliseconds)", asFUNCTION(__FadeOutSample), asCALL_GENERIC); assert(r >= 0);
